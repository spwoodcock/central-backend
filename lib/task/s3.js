...
/**
 * Uploads pending blobs to S3 using an existing dependency container.
 *
 * This function is intended for use in HTTP API routes or other internal code
 * where a shared container (including a pre-initialized DB pool) is already available.
 * It avoids creating new database connections.
 *
 * @param {Object} container - The dependency injection container with initialized services.
 * @param {Object} container.s3 - S3 client instance.
 * @param {Object} container.Blobs - Blob data access layer.
 * @param {number} [limit] - Optional limit on number of blobs to upload.
 * @returns {Promise<void>}
 */
const uploadPendingViaApi = async ({ s3, Blobs }, limit) => {
  assertEnabled(s3);
...
/**
 * CLI-compatible wrapper that sets up its own container (including DB connection)
 * before uploading pending blobs to S3.
 *
 * Intended for use in task scripts or cron jobs where no shared container exists.
 *
 * @param {number} [limit] - Optional limit on number of blobs to upload.
 * @returns {Promise<void>}
 */
const uploadPending = withContainer((container) => async (limit) =>
  uploadPendingViaApi(container, limit)
);

module.exports = { getCount, setFailedToPending, uploadPending, uploadPendingViaApi };
